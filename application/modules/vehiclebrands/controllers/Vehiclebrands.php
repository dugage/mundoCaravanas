...
class Vehiclebrands  extends MX_Controller
{
	...
	public function __construct()
	{
		...
		$this->nameModule = "Marcas";
		$this->breadCrumbs = array('Marcas');
		/*
	...
	public function index()
	{
        ...
        $data['getResult'] = $this->doctrine->em->getRepository("Entities\\VehicleBrands")->findAll();
	...
	public function add()
	{
		...
		array_push($this->breadCrumbs, "Nueva marca");
		//pasamos los datos básicos a la vista
		$data = $this->data;
		//pasamos los datos básicos a la vista
        $data['content'] = strtolower(__FUNCTION__."_".$this->nameClass);
	...
	{
		//añadirmos a las migas de pan la sección donde nos encontramos actualmente
		array_push($this->breadCrumbs, "Editar marca");
		//pasamos los datos básicos a la vista
		$data = $this->data;
		//pasamos los datos básicos a la vista
        $data['content'] = strtolower(__FUNCTION__."_".$this->nameClass);
        ...
        $data['getResult'] = $this->doctrine->em->find("Entities\\VehicleBrands", $id);
        //comprobamos formulario submit
        if( isset($_POST['submit-form']) ) {
        	//validamos los datos // EDITADO
            $this->form_validation->set_rules('name', 'Nombre', 'required');
	...
	public function delete($id = 0)
	{
		if( $id > 0 ){
	        ...
	        $getRow = $this->doctrine->em->find("Entities\\VehicleBrands", $id);
	        //eliminamos el item
	...
	private function _setAndGetData($id = 0)
	{
		...
		if( $id == 0 ) {
			...
			$item = new Entities\VehicleBrands;

		}else {

			//realizamos una consulta pasando el id para obtener el item que vamos a editar
			$item = $this->doctrine->em->find("Entities\\VehicleBrands", $id);
		}
		//seteamos los datos
